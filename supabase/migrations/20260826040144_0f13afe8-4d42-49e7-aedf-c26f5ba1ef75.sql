...
BEGIN
  ...
  payload := jsonb_build_object(
    'kind', 'client', 'external_id', NEW.id, 'email', NEW.email,
    'full_name', NEW.first_name || ' ' || NEW.last_name, 'phone', NEW.phone,
    'event_date', NEW.event_date, 'event_type', NEW.event_type,
    'venue_location', NEW.event_location, 'package_name', NEW.package_name, 'notes', NEW.notes
  );

  PERFORM net.http_post(
    url := 'https://atxceyzigqadmsasxags.supabase.co/functions/v1/push-to-planner',
    headers := jsonb_build_object(
      'Content-Type', 'application/json', 'Lovable-Context', 'cron',
      'x-trigger-key', (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'push_trigger_key')
    ),
    body := payload,
...
SET search_path TO ''
AS $function$
DECLARE payload jsonb; client_id uuid;
BEGIN
  SELECT id INTO client_id FROM public.profiles WHERE user_id = NEW.user_id LIMIT 1;
  IF client_id IS NULL THEN RETURN NEW; END IF;

  payload := jsonb_build_object(
    'kind', 'music', 'client_external_id', client_id, 'external_id', NEW.id,
    'request_type', NEW.request_type, 'song_title', NEW.song_title,
    'artist', NEW.artist, 'notes', NEW.notes
  );
